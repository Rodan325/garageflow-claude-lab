...
**Légal** : pages publiques `/legal` · `/privacy` · `/terms` · `/pilot-agreement` · `/dpa` (config + versions : `src/config/legal.ts`) · acceptation obligatoire versionnée (table `legal_acceptances`, `LegalAcceptanceGate`, statut : `/pro/legal-status`) · [LEGAL_READINESS_CHECKLIST](./LEGAL_READINESS_CHECKLIST.md) · [TERMS_OF_USE](./TERMS_OF_USE.md) · [PILOT_AGREEMENT_DRAFT](./PILOT_AGREEMENT_DRAFT.md) · [DPA_DRAFT](./DPA_DRAFT.md) · [PRIVACY_POLICY](./PRIVACY_POLICY.md) · [DATA_PRIVACY_NOTES](./DATA_PRIVACY_NOTES.md)
