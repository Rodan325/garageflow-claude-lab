...
const legal: LegalDocumentSource = {
  ...
  sections: [
    ...
    section('hosting', text('3. Hébergement et infrastructure', '3. Hosting and infrastructure', '3. الاستضافة والبنية التحتية'), [text(
      'L’interface web est déployée par Vercel Inc. La base de données, l’authentification, les API et le stockage technique sont fournis par Supabase, avec une région principale configurée à Paris (eu-west-3). Squarespace sert à administrer le domaine et le DNS, et éventuellement le site vitrine ; Google Workspace sert à la messagerie professionnelle. Ces usages administratifs ne signifient pas que ces services reçoivent toutes les données Clikarage. Leurs rôles contractuels exacts restent à confirmer avant l’entrée en vigueur.',
      'The web interface is deployed by Vercel Inc. Database, authentication, APIs and technical storage are provided by Supabase, with the primary region configured in Paris (eu-west-3). Squarespace is used to administer the domain and DNS, and potentially the showcase site; Google Workspace is used for professional email. These administrative uses do not mean that those services receive all Clikarage data. Their exact contractual roles remain to be confirmed before effectiveness.',
      'تُنشر واجهة الويب بواسطة Vercel Inc. وتوفر Supabase قاعدة البيانات والمصادقة وواجهات API والتخزين التقني، مع إعداد المنطقة الرئيسية في باريس (eu-west-3). ويُستخدم Squarespace لإدارة النطاق وDNS وربما الموقع التعريفي، بينما يُستخدم Google Workspace للبريد المهني. ولا تعني هذه الاستخدامات الإدارية أن هذه الخدمات تتلقى جميع بيانات Clikarage. وتظل أدوارها التعاقدية الدقيقة بحاجة إلى تأكيد قبل دخول النص حيز النفاذ.',
    )]),
...
const privacy: LegalDocumentSource = {
  ...
  sections: [
    ...
    section('recipients', text('6. Destinataires et prestataires', '6. Recipients and providers', '6. المستلمون والمزودون'), [text(
      'Les données sont accessibles aux utilisateurs habilités du garage, à l’automobiliste pour son propre dossier, à RODANBTECH dans la limite de ses missions et aux prestataires techniques nécessaires. Supabase fournit la base, l’authentification, les API et le stockage technique ; Vercel fournit le frontend et le CDN. Squarespace administre le domaine et le DNS, et Google Workspace la messagerie professionnelle, sans être présentés comme destinataires de toutes les données Clikarage. Stripe, les fournisseurs d’IA, SMS, DMS et email transactionnel ne sont pas actifs dans cette version.',
      'Data is accessible to authorised garage users, the motorist for their own record, RODANBTECH within its duties and necessary technical providers. Supabase provides database, authentication, APIs and technical storage; Vercel provides the frontend and CDN. Squarespace administers the domain and DNS, and Google Workspace professional email, without being presented as recipients of all Clikarage data. Stripe, AI, SMS, DMS and transactional-email providers are not active in this version.',
      'تتاح البيانات لمستخدمي الورشة المخولين ولصاحب المركبة في ملفه ولـRODANBTECH ضمن مهامها ولمزودي التقنية الضروريين. توفر Supabase قاعدة البيانات والمصادقة وواجهات API والتخزين التقني، وتوفر Vercel الواجهة وCDN. ويدير Squarespace النطاق وDNS، ويدير Google Workspace البريد المهني، من دون تقديمهما كجهتين تتلقيان جميع بيانات Clikarage. ولا تنشط Stripe أو مزودات الذكاء الاصطناعي أو الرسائل النصية أو DMS أو البريد المعاملاتي في هذا الإصدار.',
    )]),
...
const subprocessors: LegalDocumentSource = {
  ...
  sections: [
    ...
    section('domain-email', text('3. Domaine et messagerie', '3. Domain and email', '3. النطاق والبريد'), [text(
      'Squarespace est utilisé pour le domaine, le DNS et éventuellement le site vitrine ; Google Workspace est utilisé pour la messagerie professionnelle. Ils ne sont pas réputés traiter toutes les données Clikarage. Leurs entités contractantes, rôles de traitement et DPA éventuels doivent être confirmés depuis les comptes fournisseurs avant l’entrée en vigueur du registre.',
      'Squarespace is used for the domain, DNS and potentially the showcase site; Google Workspace is used for professional email. They are not presumed to process all Clikarage data. Their contracting entities, processing roles and any applicable DPAs must be confirmed from the provider accounts before the register becomes effective.',
      'يُستخدم Squarespace للنطاق وDNS وربما الموقع التعريفي، ويُستخدم Google Workspace للبريد المهني. ولا يُفترض أنهما يعالجان جميع بيانات Clikarage. ويجب تأكيد الكيانات المتعاقدة وأدوار المعالجة وأي اتفاقات DPA مطبقة من حسابات المزودين قبل دخول السجل حيز النفاذ.',
    )]),
