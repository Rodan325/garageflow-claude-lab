...
export function ClientNewsPage() {
  ...
  if (!selectedGarageId) {
    return (
      <div className="p-4">
        <EmptyState title="Aucun garage sélectionné" action={<Link to="/app"><Button>Choisir un garage</Button></Link>} />
      </div>
    )
  }
    ...
    <div className="p-4">
      ...
      <div className="mt-4">
        {isLoading ? (
          ...
          <EmptyState icon={Newspaper} title="Aucune actualité" />
        ) : (
          <motion.div variants={listStagger} initial="hidden" animate="show" className="space-y-3">
            {news!.map((n) => (
              <motion.div key={n.id} variants={listItem}>
                <Card className="p-4">
                  <p className="flex items-center gap-2 font-semibold"><Megaphone className="h-4 w-4 text-primary" /> {n.title}</p>
                  {n.body && <p className="mt-1.5 text-sm text-muted-foreground">{n.body}</p>}
                  <p className="mt-2 text-xs text-muted-foreground">{shortDate(n.published_at)}</p>
                </Card>
              </motion.div>
            ))}
          </motion.div>
