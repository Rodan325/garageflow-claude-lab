...
- [x] Migration
  ...
  appliquee seule sur staging apres un dry-run limite a cette version ;
  historique final 40/40 et dry-run final vide.
...
- [x] Huit acceptations historiques simulees preservees localement et champs
  ajoutes laisses `NULL` pendant la validation de compatibilite. Staging
  contenait zero preuve avant et apres la migration RPC ; aucune preservation
  de huit lignes distantes n'est revendiquee.
...
- [x] Nouveau teardown baseline/finally valide sur Docker local et staging :
  un rappel marque a provoque l'echec attendu, le `finally` a execute le
  nettoyage administratif et le baseline final a ete restaure exactement.
