...
The historical `0004_seed.sql` file mixed demonstration fixtures with migration history. Its `client_vehicles` insert declared `fuel, mileage` but supplied `98000, 'Diesel'`, so a clean PostgreSQL bootstrap attempted to cast `Diesel` to an integer and failed.

## Resolution

`0004_seed.sql` keeps its historical filename and is now an intentional no-op marker. All useful fictitious records were moved to `supabase/seed.sql`, where they run after the complete schema and use deterministic identifiers plus conflict handling. The obsolete anonymous pending request was not moved because the current seed already contains explicit independent-garage lifecycle scenarios.
