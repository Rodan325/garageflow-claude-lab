-- Bind current legal evidence to the exact localized document displayed.
-- Existing evidence remains immutable: this migration performs no data rewrite,
-- backfill, deletion, or historical migration change.
...
begin
  ...
  where document.document_id = p_document_key
    ...
    and document.requires_acceptance is true
  ...
  if v_document.acceptance_scope = 'organization' then
    ...
    where acceptance.organization_id = p_organization_id
      ...
      and acceptance.displayed_language = v_document.language
      and acceptance.document_sha256 = v_document.sha256
      and acceptance.document_status = 'effective'
      and acceptance.acceptance_scope = v_document.acceptance_scope
...
begin
  ...
  where document.document_id = v_status.document_key
    ...
    and document.effective_at <= pg_catalog.now()
    and document.requires_acceptance is true
    and document.acceptance_scope = v_status.acceptance_scope
  for share;

  -- Locale and hash are part of the evidence identity. Concurrent acceptances
  -- for another translation must not reuse this localized proof.
...
create unique index if not exists legal_acceptances_v2_user_locale_unique
...
create unique index if not exists legal_acceptances_v2_organization_locale_unique
  on public.legal_acceptances (
    organization_id,
    document_id,
    document_version,
...
comment on function public.get_current_legal_acceptance_status_v2(text, text, uuid) is
  'Returns current evidence only when document key, version, locale, canonical hash, scope and subject exactly match.';
comment on function public.accept_current_legal_document_v2(text, text, uuid) is
  'Records the exact server-resolved current localized legal document; actor, timestamp, version, hash and scope are never caller supplied.';
